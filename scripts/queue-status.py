...
Reads:
  ...
  - data/technical-candidates.tsv  (the source of truth for "what could become a post")
...
CANDIDATES_FILE = REPO_ROOT / "data" / "technical-candidates.tsv"
...
def candidates() -> list[tuple[str, str, str]]:
    """Return [(id, title, duration), ...] from technical-candidates.tsv."""
...
def main() -> int:
    ...
    print(f"Technical candidates (from filter): {len(cand)}")
